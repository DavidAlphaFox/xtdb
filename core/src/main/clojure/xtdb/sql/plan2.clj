...
(defn- project-all-cols [env scope order-by-specs]
  ...
  {:projected-cols (as-> (vec (for [col-name (available-cols scope nil)
                     ...
                     (into projected-cols (extended-ob-col-refs order-by-specs env scope (into #{} (map (comp str :col-sym)) projected-cols))))})
...
(defrecord ExprPlanVisitor [env scope]
  ...
  (visitColumnReference [{:keys [^Set !ob-col-refs]} ctx]
    (let [schema-name (identifier-sym (.schemaName ctx))
  ...
  (visitPeriodColumnReference [_ ctx]
    (let [tn (identifier-sym (.tableName ctx))
      ...
      (case pcn
        "VALID_TIME" (if tn
                       {:from (find-decl scope tn 'xt$valid_from)
                        :to (find-decl scope tn 'xt$valid_to)}
                       {:from (find-decl scope 'xt$valid_from)
                        :to (find-decl scope 'xt$valid_to)})
        "SYSTEM_TIME" (if tn
                        {:from (find-decl scope tn 'xt$system_from)
                         :to (find-decl scope tn 'xt$system_to)}
                        {:from (find-decl scope 'xt$system_from)
                         :to (find-decl scope 'xt$system_to)}))))
